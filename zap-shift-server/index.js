...
// Secret used to sign OTP tokens — add OTP_JWT_SECRET to your .env
// e.g. OTP_JWT_SECRET=some_long_random_string
const OTP_JWT_SECRET = process.env.OTP_JWT_SECRET || "otp_secret_key_change_in_production";
...
async function connectDB() {
  ...
  await parcelsCollection.createIndex({ trackingId: 1 }, { unique: true, sparse: true });
...
/* -------------------------------------------------------------------------- */
/*                         OTP Routes (JWT — No Database)                     */
/* -------------------------------------------------------------------------- */

// STEP 1: Generate OTP → sign as JWT → send email → return token to frontend
app.post("/auth/send-otp", async (req, res) => {
  try {
    ...
    // Sign OTP + email into a JWT that expires in 5 minutes
    // The OTP is stored inside the token — no database needed
    ...
    // Return the token to the frontend — frontend holds it in state
    res.send({
      success: true,
      message: "OTP sent successfully",
      token, // frontend stores this and sends it back on verify
    });
  } catch (error) {
...
// STEP 2: Verify OTP — decode the token, check email + otp match
app.post("/auth/verify-otp", async (req, res) => {
  try {
    ...
    // Verify and decode the JWT — this also checks expiry automatically
    let decoded;
    try {
      decoded = jwt.verify(token, OTP_JWT_SECRET);
    } catch (err) {
    ...
    // Check email and otp match what's inside the token
...
app.post("/create-payment-intent", verifyFBToken, async (req, res) => {
  try {
    ...
    if (amountInCents < 50) {
      return res.status(400).json({ message: "Minimum payable amount is 60 Tk (0.50 USD)" });
    }
    ...
    res.json({ clientSecret: paymentIntent.client_secret, amountInCents, usdAmount, takaAmount });
  } catch (error) {
...
app.post("/payments", verifyFBToken, async (req, res) => {
  try {
    ...
    const { parcelId, transactionId, amountTaka, amountUsd, email, paymentMethodId, paymentMethod, paymentIntentId, status } = paymentInfo;
...
app.get("/payments/:email", verifyFBToken, async (req, res) => {
  try {
    ...
    const payments = await paymentsCollection.find({ email }).sort({ paidAt: -1 }).toArray();
    res.json(payments);
  } catch (error) {
...
app.post("/rider-accounts", verifyFBToken, async (req, res) => {
  try {
    ...
    if (riderExists) {
      return res.status(200).send({ message: "Rider profile already exists", inserted: false, riderExists: true });
    }

    const riderDoc = {
      name: name || "", email,
      age: age || "", phone: phone || "",
      nid: nid || "", region: region || "",
      hub: hub || "", vehicleType: vehicleType || "",
      picture: picture || "",
      role: "rider", status: "active",
      approvalStatus: "pending", workStatus: "free",
      created_at: now(), updated_at: now(),
    };

    const result = await riderAccountsCollection.insertOne(riderDoc);

    await usersCollection.updateOne({ email }, { $set: { role: "rider", updated_at: now() } });
    ...
    res.status(201).send({ message: "Rider profile created successfully", inserted: true, result });
  } catch (error) {
    res.status(500).send({ error: error.message });
  }
});
...
app.patch("/rider-accounts/profile", verifyFBToken, async (req, res) => {
  try {
    const { email, name, age, phone, picture, vehicleType, nid, hub, region, status } = req.body;
    ...
    const result = await riderAccountsCollection.updateOne(
      { email },
      {
        $set: {
          ...
          vehicleType: vehicleType || "", nid: nid || "",
    ...
    res.status(500).send({ message: "Failed to update rider profile", error: error.message });
  }
});
...
const createRiderTask = async ({ parcel, rider, adminEmail, adminMessage = "" }) => {
  ...
  const newTask = {
    ...
    senderInfo: {
      ...
      address: parcel.senderAddress || "", center: parcel.senderCenter || "",
    },
    receiverInfo: {
      name: parcel.receiverName || "", phone: parcel.receiverPhone || "",
      address: parcel.receiverAddress || "", center: parcel.receiverCenter || "",
    },
    parcelInfo: {
...
app.patch("/admin/parcels/:parcelId/assign-rider", verifyFBToken, verifyAdmin, async (req, res) => {
  try {
    const { parcelId } = req.params;
    const { riderId, message } = req.body;

    if (!parcelId || !riderId) {
      return res.status(400).send({ message: "parcelId and riderId are required" });
    }
    ...
    if (parcel.assignedRiderEmail) return res.status(400).send({ message: "Parcel is already assigned to a rider" });
    ...
    if (rider.approvalStatus !== "approved") return res.status(400).send({ message: "Rider is not approved" });
    if (rider.workStatus === "busy") return res.status(400).send({ message: "Rider is already busy" });

    const result = await createRiderTask({ parcel, rider, adminEmail: req.decoded.email, adminMessage: message || "" });

    res.status(201).send({ message: "Rider assigned successfully", insertedId: result.insertedId });
  } catch (error) {
    res.status(500).send({ message: "Failed to assign rider", error: error.message });
  }
});
...
app.post("/rider-tasks", verifyFBToken, verifyAdmin, async (req, res) => {
  try {
    ...
    if (!parcelId || !riderEmail) {
      return res.status(400).send({ message: "parcelId and riderEmail are required" });
    }
    ...
    const result = await createRiderTask({ parcel, rider, adminEmail: req.decoded.email, adminMessage: req.body.message || "" });

    res.status(201).send({ message: "Rider task created successfully", insertedId: result.insertedId });
  } catch (error) {
    res.status(500).send({ message: "Failed to create rider task", error: error.message });
  }
});
...
app.get("/rider-tasks/rider/:email", verifyFBToken, async (req, res) => {
  try {
    ...
    const tasks = await riderTasksCollection.find({ riderEmail: email }).sort({ assignedAt: -1 }).toArray();
    res.send(tasks.map(serializeDoc));
  } catch (error) {
    res.status(500).send({ error: error.message });
  }
...
app.patch("/rider-tasks/:id", verifyFBToken, verifyRiderOrAdmin, async (req, res) => {
  try {
    ...
    await riderTasksCollection.updateOne(
      { _id: toObjectId(id) },
      { $set: { status: nextStatus, updatedAt: now(), ...(nextStatus === "completed" ? { completedAt: now() } : {}) } }
    );

    const parcelStatusMap = {
      assigned: "assigned", taken: "taken", shifted: "shifted",
      "out for delivery": "out for delivery", completed: "completed", cancelled: "cancelled",
    };

    await parcelsCollection.updateOne(
      { _id: toObjectId(task.parcelId) },
      { $set: { deliveryStatus: parcelStatusMap[nextStatus] || nextStatus, parcelCurrentStatus: parcelStatusMap[nextStatus] || nextStatus, updatedAt: now() } }
    );

    if (nextStatus === "completed") {
      await riderAccountsCollection.updateOne({ email: task.riderEmail }, { $set: { workStatus: "free", updated_at: now() } });

      const existingEarning = await riderEarningsCollection.findOne({ parcelId: task.parcelId, riderEmail: task.riderEmail });

      if (!existingEarning) {
        await riderEarningsCollection.insertOne({
          riderEmail: task.riderEmail, riderName: task.riderName || "",
          parcelId: task.parcelId, trackingId: task.trackingId || "",
          amount: Number(amount || 50), status: "unpaid",
    ...
    if (nextStatus === "cancelled") {
      ...
      await parcelsCollection.updateOne(
        { _id: toObjectId(task.parcelId) },
        { $set: { assignedRiderId: null, assignedRiderEmail: null, assignedRiderName: null, deliveryStatus: "pending", parcelCurrentStatus: "pending", updatedAt: now() } }
      );
    }

    await createNotification({
      type: "rider_task_update", title: "Rider task updated",
      message: `Rider updated parcel status to ${nextStatus}`,
      recipientRole: "admin", relatedId: task.parcelId,
      relatedCollection: "riderTasks", meta: { riderEmail: task.riderEmail, status: nextStatus },
    });

    res.send({ message: "Task status updated successfully" });
  } catch (error) {
    res.status(500).send({ message: "Failed to update rider task", error: error.message });
  }
});
...
app.post("/rider-earnings", verifyFBToken, verifyAdmin, async (req, res) => {
  try {
    ...
    if (!riderEmail || !parcelId) {
      return res.status(400).send({ message: "riderEmail and parcelId are required" });
    }

    const existing = await riderEarningsCollection.findOne({ riderEmail, parcelId });
    if (existing) return res.status(200).send({ message: "Rider earning already exists", existing: true });

    const newEarning = { ...req.body, amount: Number(amount || 0), status: req.body.status || "unpaid", createdAt: now(), updatedAt: now() };
    const result = await riderEarningsCollection.insertOne(newEarning);

    res.status(201).send({ message: "Rider earning saved successfully", insertedId: result.insertedId });
  } catch (error) {
    res.status(500).send({ message: "Failed to save rider earning", error: error.message });
  }
});
...
app.get("/rider-earnings/:email", verifyFBToken, async (req, res) => {
  try {
    ...
    const earnings = await riderEarningsCollection.find({ riderEmail: email }).sort({ createdAt: -1 }).toArray();
...
app.get("/rider-earnings-summary/:email", verifyFBToken, async (req, res) => {
  try {
    ...
    const paidEarnings = earnings.filter((item) => item.status === "paid").reduce((sum, item) => sum + Number(item.amount || 0), 0);
    const unpaidEarnings = earnings.filter((item) => item.status === "unpaid").reduce((sum, item) => sum + Number(item.amount || 0), 0);

    res.send({ totalEarnings, paidEarnings, unpaidEarnings, totalRecords: earnings.length });
  } catch (error) {
    res.status(500).send({ error: error.message });
  }
});
...
app.get("/admin/overview", verifyFBToken, verifyAdmin, async (req, res) => {
  try {
    ...
    const [pendingParcels, completedParcels, pendingRiders, availableRiders, busyRiders, unpaidOrders] = await Promise.all([
      ...
      riderAccountsCollection.countDocuments({ approvalStatus: "approved", workStatus: "free" }),
      riderAccountsCollection.countDocuments({ approvalStatus: "approved", workStatus: "busy" }),
    ...
    const totalCashIn = successfulPayments.reduce((sum, payment) => sum + Number(payment.amountTaka || 0), 0);

    const paidRiderEarnings = await riderEarningsCollection.find({ status: "paid" }).toArray();
    const totalCashOut = paidRiderEarnings.reduce((sum, earning) => sum + Number(earning.amount || 0), 0);

    const recentNotifications = await notificationsCollection.find({ recipientRole: "admin" }).sort({ createdAt: -1 }).limit(8).toArray();

    res.send({
      stats: { totalUsers, totalRiders, totalParcels, totalPayments, pendingParcels, completedParcels, pendingRiders, availableRiders, busyRiders, unpaidOrders, totalCashIn, totalCashOut },
      recentNotifications,
    });
  } catch {
...
app.get("/admin/users", verifyFBToken, verifyAdmin, async (req, res) => {
  try {
    ...
    const query = search
      ? { $or: [{ name: { $regex: search, $options: "i" } }, { email: { $regex: search, $options: "i" } }, { phone: { $regex: search, $options: "i" } }], role: { $ne: "admin" } }
      : { role: { $ne: "admin" } };
    ...
    const enrichedUsers = await Promise.all(
      users.map(async (user) => {
        const totalParcels = await parcelsCollection.countDocuments({ userEmail: user.email });
        return { ...serializeDoc(user), totalParcels, accountStatus: user.status || "active" };
      })
    );
...
app.get("/admin/users/:id", verifyFBToken, verifyAdmin, async (req, res) => {
  try {
    ...
    res.send({ user: serializeDoc(user), parcelHistory: parcelHistory.map(serializeDoc), paymentHistory: paymentHistory.map(serializeDoc) });
  } catch {
...
app.get("/admin/orders/:id", verifyFBToken, verifyAdmin, async (req, res) => {
  try {
    ...
    const payment = order.transactionId ? await paymentsCollection.findOne({ transactionId: order.transactionId }) : null;
...
app.get("/admin/parcel-tracking", verifyFBToken, verifyAdmin, async (req, res) => {
  try {
    const search = req.query.search || "";
    const query = search
      ? { $or: [{ trackingId: { $regex: search, $options: "i" } }, { senderName: { $regex: search, $options: "i" } }, { receiverName: { $regex: search, $options: "i" } }] }
      : {};
...
app.patch("/admin/parcel-tracking/:id/status", verifyFBToken, verifyAdmin, async (req, res) => {
  try {
    ...
    await parcelsCollection.updateOne({ _id: toObjectId(id) }, { $set: { deliveryStatus: normalized, parcelCurrentStatus: normalized, updatedAt: now() } });

    if (parcel.assignedRiderEmail) {
      await riderTasksCollection.updateOne(
        { parcelId: id, riderEmail: parcel.assignedRiderEmail },
        { $set: { status: normalized, updatedAt: now(), ...(normalized === "completed" ? { completedAt: now() } : {}) } }
      );
    }

    if (normalized === "completed" && parcel.assignedRiderEmail) {
      await riderAccountsCollection.updateOne({ email: parcel.assignedRiderEmail }, { $set: { workStatus: "free", updated_at: now() } });
    }

    await createNotification({
      type: "parcel_status_update", title: "Parcel status updated",
      message: `Parcel ${parcel.trackingId || id} updated to ${normalized}`,
      recipientRole: "admin", relatedId: id, relatedCollection: "parcels", meta: { status: normalized },
    });
  ...
  try {
    ...
    const items = await Promise.all(
      payments.map(async (payment) => {
        const parcel = isValidObjectId(payment.parcelId) ? await parcelsCollection.findOne({ _id: toObjectId(payment.parcelId) }) : null;
    ...
    const pendingAdminReceive = payments.filter((p) => p.cashInStatus === "pending_admin_receive").length;

    res.send({ summary: { totalCashIn, paidCount, pendingAdminReceive }, payments: items });
  } catch {
...
app.patch("/admin/payments/:id/receive", verifyFBToken, verifyAdmin, async (req, res) => {
  try {
    ...
    await paymentsCollection.updateOne({ _id: toObjectId(id) }, { $set: { cashInStatus: "received_by_admin", cashReceivedAt: now() } });

    if (payment.parcelId && isValidObjectId(payment.parcelId)) {
      await parcelsCollection.updateOne({ _id: toObjectId(payment.parcelId) }, { $set: { cashReceivedByAdmin: true, updatedAt: now() } });
    }
...
app.get("/admin/parcels/unassigned", verifyFBToken, verifyAdmin, async (req, res) => {
  try {
    const parcels = await parcelsCollection.find({
      $and: [
        { $or: [{ assignedRiderId: null }, { assignedRiderId: { $exists: false } }, { assignedRiderId: "" }] },
        { $or: [{ assignedRiderEmail: null }, { assignedRiderEmail: { $exists: false } }, { assignedRiderEmail: "" }] },
    ...
    res.status(500).send({ message: "Failed to fetch unassigned parcels", error: error.message });
  }
});

app.get("/admin/riders/available", verifyFBToken, verifyAdmin, async (req, res) => {
  try {
    const riders = await riderAccountsCollection.find({ approvalStatus: "approved", workStatus: "free" }).toArray();
    res.send(riders);
  } catch {
...
app.get("/admin/riders/:id", verifyFBToken, verifyAdmin, async (req, res) => {
  try {
    ...
    res.send({ rider: serializeDoc(rider), tasks: tasks.map(serializeDoc), earnings: earnings.map(serializeDoc) });
  } catch {
...
app.patch("/admin/riders/:id/approval", verifyFBToken, verifyAdmin, async (req, res) => {
  try {
    ...
    await riderAccountsCollection.updateOne({ _id: toObjectId(id) }, { $set: updateDoc });

    await createNotification({
      type: "rider_approval", title: "Rider approval updated",
      message: `Rider ${rider.email} status changed to ${normalized}`,
      recipientRole: "admin", relatedId: id, relatedCollection: "riderAccounts", meta: { approvalStatus: normalized },
    });
  ...
  try {
    ...
    const result = await Promise.all(
      riders.map(async (rider) => {
        const earnings = await riderEarningsCollection.find({ riderEmail: rider.email }).toArray();
        const totalPayment = earnings.reduce((sum, item) => sum + Number(item.amount || 0), 0);
        const paidAmount = earnings.filter((item) => item.status === "paid").reduce((sum, item) => sum + Number(item.amount || 0), 0);

        return {
          riderId: rider._id.toString(), riderName: rider.name || "",
          riderEmail: rider.email, completedParcels: earnings.length,
          totalPayment, paidAmount, dueAmount: totalPayment - paidAmount,
          approvalStatus: rider.approvalStatus, workStatus: rider.workStatus,
        };
      })
    );
...
app.patch("/admin/rider-payments/pay", verifyFBToken, verifyAdmin, async (req, res) => {
  try {
    ...
    if (!riderEmail) return res.status(400).send({ message: "riderEmail is required" });

    const unpaidEarnings = await riderEarningsCollection.find({ riderEmail, status: "unpaid" }).toArray();
    if (!unpaidEarnings.length) return res.status(400).send({ message: "No unpaid earnings found" });

    const totalPaidNow = unpaidEarnings.reduce((sum, item) => sum + Number(item.amount || 0), 0);

    await riderEarningsCollection.updateMany({ riderEmail, status: "unpaid" }, { $set: { status: "paid", paidAt: now(), updatedAt: now() } });

    await createNotification({
      type: "cash_out", title: "Cash out message",
      message: `Cash out completed for rider ${riderEmail}`,
      recipientRole: "admin", relatedCollection: "riderEarnings", meta: { riderEmail, totalPaidNow },
    });
  ...
  try {
    ...
    const enrichedTasks = await Promise.all(
      tasks.map(async (task) => {
        ...
        const parcel = task.parcelId && ObjectId.isValid(task.parcelId) ? await parcelsCollection.findOne({ _id: toObjectId(task.parcelId) }) : null;
        return { ...task, availability: rider?.workStatus || "unknown", parcelName: parcel?.parcelName || parcel?.trackingId || "" };
      })
    );
...
app.get("/admin/notifications", verifyFBToken, verifyAdmin, async (req, res) => {
  try {
    const notifications = await notificationsCollection.find({ recipientRole: "admin" }).sort({ createdAt: -1 }).toArray();
...
app.patch("/admin/notifications/:id/read", verifyFBToken, verifyAdmin, async (req, res) => {
  try {
    ...
    await notificationsCollection.updateOne({ _id: toObjectId(id) }, { $set: { isRead: true, readAt: now() } });
...
app.get("/admin/dashboard-overview", verifyFBToken, verifyAdmin, async (req, res) => {
  try {
    const [totalUsers, totalRiders, totalParcels, completedParcels, pendingParcels, pendingRiders, availableRiders, unpaidOrders] = await Promise.all([
      usersCollection.countDocuments({ role: { $ne: "admin" } }),
      riderAccountsCollection.countDocuments(),
      parcelsCollection.countDocuments(),
      parcelsCollection.countDocuments({ deliveryStatus: "completed" }),
    ...
    const notifications = await notificationsCollection.find({ recipientRole: "admin" }).sort({ createdAt: -1 }).limit(6).toArray();

    res.send({
      stats: { totalUsers, totalRiders, totalParcels, completedParcels, pendingParcels, pendingRiders, availableRiders, unpaidOrders, totalCashIn, totalCashOut },
      notifications,
    });
  } catch {
...
app.get("/rider/notifications", verifyFBToken, async (req, res) => {
  try {
    ...
    const notifications = await notificationsCollection.find({ recipientRole: "rider", recipientEmail: email }).sort({ createdAt: -1 }).limit(100).toArray();
    res.send(notifications.map(serializeDoc));
  } catch {
    res.status(500).send({ message: "Failed to fetch notifications" });
  }
...
app.get("/rider/notifications/unread-count", verifyFBToken, async (req, res) => {
  try {
    ...
    const count = await notificationsCollection.countDocuments({ recipientRole: "rider", recipientEmail: email, isRead: false });
    res.send({ count });
  } catch {
...
app.patch("/rider/notifications/read-all", verifyFBToken, async (req, res) => {
  try {
    ...
    await notificationsCollection.updateMany({ recipientRole: "rider", recipientEmail: email, isRead: false }, { $set: { isRead: true, readAt: now() } });
...
app.patch("/rider/notifications/:id/read", verifyFBToken, async (req, res) => {
  try {
    ...
    const notification = await notificationsCollection.findOne({ _id: toObjectId(id) });
    if (!notification) return res.status(404).send({ message: "Notification not found" });
    if (notification.recipientEmail !== email) return res.status(403).send({ message: "forbidden access" });
