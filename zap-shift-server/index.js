...
if (!process.env.EMAIL_SENDER || !process.env.EMAIL_PASS) {
  console.error("EMAIL_SENDER or EMAIL_PASS is missing");
  process.exit(1);
}
...
const sendOtpEmail = async (email, otp) => {
  const mailOptions = {
    ...
    to: email,
    subject: "Your Profast OTP Code",
    text: `Your OTP is ${otp}. This OTP will expire in 5 minutes.`,
    html: `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #222;">
        <h2 style="margin-bottom: 8px;">Profast Email Verification</h2>
...
app.post("/auth/send-otp", async (req, res) => {
  try {
    ...
    const mailInfo = await sendOtpEmail(email, otp);
...
app.patch(
  ...
  async (req, res) => {
    try {
      ...
      if (parcel.assignedRiderEmail) {
        return res.status(400).send({ message: "Parcel is already assigned to a rider" });
      }
